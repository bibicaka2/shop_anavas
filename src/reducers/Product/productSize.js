...
const product_size = (state = initialState, action)=>{
    var {item,product}=action;
    switch(action.type)
    {
        
        case Types.FETCH_PRODUCTS_SIZE:    
      
        state=action.products;
        
        return [...state];
        
        case Types.ADD_TO_PRODUCT_SIZE:    
        ...
        if(product!==null)
        {
           ...
           // http://localhost:8000/api/Products/create?name=ao&img=https://ananas.vn/wp-content/uploads/AGT0011_1-1.jpg&price=2000&status_id=SHIRT&description=aab&product_size_list[0][size_id]=S&product_size_list[0][inventory]=15&sale_id=123&rating=2&color=black&product_type_id=123&price_origin=20000&price_after_sale_off=210000&gender=NỮ
            ...
            var add_product_size_Api= `ProductSize/create?name=${product.name}`
            callApi(`${add_product_size_Api}`,'POST',null);
  
        }
        return state;

        case Types.UPDATE_TO_PRODUCT_SIZE:  
        if(item!==null)
        {
        ...
        //    });
             var update_product_size_Api= `ProductSize/update?_id=${item._id}&name=${item.name}`
            callApi(`${update_product_size_Api}`,'POST',null);
  
        }
        return state;
      
        case Types.DELETE_TO_PRODUCT_SIZE:    
        console.log('vo redux')
        callApi(`ProductSize/delete?_id=${item._id}`, 'POST', null).then(res => {
        });
...
//     var link_1=`&color=${product.color}&gender=${product.gender}&sale_id=${product.sale_id}&product_type_id=${product.product_type_id}&rating=${product.rating}&name=${product.name}&img=${product.img}&price_origin=${product.price_origin}&price_after_sale_off=${product.price_after_sale_off}&status_id=${product.status_id}&description=${product.description}`
//     if(length_list>0)
//     {
...
//     var length_list=product.product_size_list.length;
//     var link_1=`&color=${product.color}&gender=${product.gender}&sale_id=${product.sale_isd}&product_type_id=${product.product_type_id}&rating=${product.rating}&name=${product.name}&img=${product.img}&price_origin=${product.price_origin}&price_after_sale_off=${product.price_after_sale_off}&status_id=${product.status_id}&description=${product.description}`
//     if(length_list>0)
//     {
